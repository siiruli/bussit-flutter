...
class MapPoint {
  ...
  MapPoint(this.color, lat, lon) : point = LatLng(lat.toDouble(), lon.toDouble()); 
}

MapPoint? mapPointFromStop(Query$Itinerary$plan$itineraries$legs$intermediateStops? stop){
  if(stop?.lat != null && stop?.lon != null) {
    return MapPoint(
      TransitMode(stop!.vehicleMode).color,
      stop.lat, stop.lon
    );
  }else{
    return null;
